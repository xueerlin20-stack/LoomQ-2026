...
也可以使用仓库内的本地配置启动器，不需要每次手动 `export`：

```bash
cp starter_kit/.env.l2.example starter_kit/.env.l2
# 编辑 starter_kit/.env.l2，把 LOOMQ_LLM_API_KEY=placeholder 换成自己的 Key
python3 starter_kit/run_l2.py --check
python3 starter_kit/run_l2.py "生成一个 3 比特 GHZ 态并进行全测量"
```

真实 `.env.l2` 已被 Git 忽略，只有不含密钥的 `.env.l2.example` 会进入提交。启动器不会显示 API Key，命令行已存在的 `LOOMQ_LLM_*` 变量优先于文件值。正式评测仍由组委会直接注入环境变量，不依赖这个本地文件。
